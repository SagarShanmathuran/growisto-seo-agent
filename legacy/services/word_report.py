...
def generate_word_report(
    ...
    # ── ROI Summary ───────────────────────────────────────────────────────────
    _section_heading(doc, "SEO ROI Estimate", "Projected return from capturing 10% of competitor traffic")
    _metric_cards(doc, [
        ("Competitor Traffic",    f"{total_comp:,} / mo"),
        ("Achievable (10%)",      f"{roi.get('achievable_traffic', 0):,} / mo"),
        ("Est. Monthly Revenue",  f"{currency_symbol}{roi.get('monthly_revenue', 0):,.0f}"),
        ("Annual ROI",            f"{roi.get('roi_pct', 0)}%"),
        ("Viable",                "YES ✓" if roi.get("is_viable") else "NO ✗"),
    ])
    ...
    if not all_gaps.empty:
        _section_heading(doc, f"Keyword Gaps  —  {len(all_gaps):,} Opportunities",
                         "Competitors rank top 20, client has no presence")
        display = all_gaps[[c for c in [
            "keyword", "search_volume", "competitor",
            "competitor_position", "client_position", "opportunity_score"
        ] if c in all_gaps.columns]].head(50).copy()
        display.columns = [c.replace("_", " ").title() for c in display.columns]
        _add_table(doc, display, max_rows=50)

    # ── Low-Hanging Fruit ─────────────────────────────────────────────────────
    if not lh_df.empty:
        _section_heading(doc, f"Low-Hanging Fruit  —  {len(lh_df):,} Keywords",
                         "Client ranks positions 11–30 — quick wins to push to page 1")
        display = lh_df[[c for c in [
            "keyword", "current_position", "search_volume",
            "traffic_if_top5", "priority_score"
        ] if c in lh_df.columns]].head(30).copy()
        display.columns = [c.replace("_", " ").title() for c in display.columns]
        _add_table(doc, display, max_rows=30)

    # ── Top Competitor Pages ───────────────────────────────────────────────────
    if not tp_df.empty:
        _section_heading(doc, "Top Competitor Pages by Traffic",
                         "High-traffic pages to replicate or out-rank")
        display = tp_df[[c for c in [
            "competitor", "url", "traffic", "top_keyword"
        ] if c in tp_df.columns]].head(20).copy()
        display.columns = [c.replace("_", " ").title() for c in display.columns]
        _add_table(doc, display, max_rows=20)

    # ── Strategic Recommendations ─────────────────────────────────────────────
    doc.add_page_break()
    _section_heading(doc, "Strategic Recommendations",
                     "Data-driven action plan based on the analysis above")
